...
class MacOSTrayService implements TrayService {
  ...
  Future<void> updateMenu(List<TrayMenuItem> items) async {
    ...
    //   if (item.isSeparator) {
    //     menu.buildFrom([MenuSeparator()]);
    //   } else {
    //     menu.buildFrom([
    //       MenuItemLabel(
    //         label: item.label,
    //         enabled: item.enabled,
    //         onClicked: (_) => _menuController.add(item.id),
    //       ),
    //     ]);
    //   }
    // }
  ...
  Future<void> setBadge(int? count) async {
    // macOS doesn't support tray badges directly
    // Use app dock badge instead via macOS native code
  }
